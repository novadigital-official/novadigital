  ...
  return (
    <section id="cozumler" className="py-12 sm:py-14 relative bg-transparent">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* 📱 MOBILE VIEW: Smooth Touch-Swipe Horizontal Card Carousel */}
        <div className="md:hidden overflow-x-auto pb-4 pt-1 flex gap-4 no-scrollbar -mx-4 px-4 snap-x snap-mandatory">
          {solutions.map((item, idx) => {
            const Icon = item.icon;
            return (
              <div
                key={idx}
                className="glass-box p-6 rounded-2xl flex-[0_0_86%] snap-center flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-500/20 text-blue-300 font-mono text-[11px] font-semibold border border-blue-500/30">
                      {item.badge}
                    </span>
                ...
                <div className="pt-3 border-t border-white/10 flex items-center justify-between">
                  <div>
                    <div className="text-[10px] text-slate-400 font-medium">{item.metrics.label}</div>
                    <div className="text-sm font-extrabold font-mono text-white">{item.metrics.value}</div>
                  </div>
                  <a
                    href="#iletisim"
                    className="btn-shimmer px-4 py-2 rounded-xl text-xs font-bold text-white shadow-md flex items-center gap-1"
                  >
                    <span>Detay</span>
        ...
        {/* 💻 DESKTOP VIEW: Interactive Tab Bar + Studio Screen with Embedded Video */}
        <div className="hidden md:block">
          ...
          <div className="glass-box p-8 rounded-2xl w-full">
            <div className="grid grid-cols-12 gap-8 items-center">
              ...
              {/* Right Column: Embedded Video & Dynamic Metric Badge */}
